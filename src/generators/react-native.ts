 ...
 * 更新Expo package.json
 */
async function updateExpoPackageJson(
    mobilePath: string,
    config: ProjectConfig
): Promise<void> {
...
`
    ...
    await fs.writeFile(
        path.join(mobilePath, 'metro.config.js'),
        metroConfigContent
    )
}
 ...
 * 创建基础组件
 */
async function createBaseComponents(
    mobilePath: string,
    config: ProjectConfig
): Promise<void> {
    // 创建组件目录结构
    const componentsDir = path.join(mobilePath, 'components')
...
`
    ...
    // 创建状态管理store
    const storeDir = path.join(mobilePath, 'store')
...
`
    ...
    await fs.writeFile(path.join(storeDir, 'index.ts'), storeContent)

    // 创建API服务
    const servicesDir = path.join(mobilePath, 'services')
...
`
    ...
    await fs.writeFile(path.join(servicesDir, 'api.ts'), apiServiceContent)
...
async function updateMainScreen(
    ...
    const appDir = path.join(mobilePath, 'app')
...
`
    ...
    await fs.writeFile(path.join(appDir, 'index.tsx'), indexPageContent)
}
